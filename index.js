...
import { addMessage, getRecentMessages, clearUser } from '../../db.js';
...
export default async function handler(req, res) {
  if (req.method === 'GET' && req.url.startsWith('/ping')) return res.status(200).send('pong');
  ...
  try {
    ...
    const history = await getRecentMessages(userId, MEMORY_TURNS);
